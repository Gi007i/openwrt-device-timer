#!/bin/sh
# schedule.sh - Schedule logic, time windows, midnight reset
# Requires: SYSTEM_TZ, LAST_DATE_FILE, log(), state.sh, firewall.sh
...
reset_device() {
	...
	# Preserves paused and flatrate flags
...
check_midnight_reset() {
	...
	if [ "$current_date" != "$last_date" ]; then
		...
		# Commit and reload immediately so devices are unblocked at midnight
