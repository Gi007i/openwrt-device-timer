#!/bin/sh
# state.sh - State management (requires: STATE_FILE, TEMP_DIR, log())

# TSV format: device_id\tusage\tprevious_usage\tlast_run_time\tactive_window\tflatrate\tpaused\tcal_status\tcal_start\tcal_idle_dur\tcal_last_sample\tcal_p2_start\tcal_usage_dur
read_all_states() {
	ALL_STATES=""
...
# Returns: start_time|idle_duration|last_sample_time|phase2_start_time|usage_duration
...
# Single-writer pattern: rpcd queues commands to state.json.rpc, daemon processes them here
process_rpc_updates() {
	local rpc_file="${STATE_FILE}.rpc"
	local processing_file="${rpc_file}.processing"
	[ -f "$rpc_file" ] || return 0

	# Atomic rename prevents TOCTOU race with rpcd (shared lock)
...
# Bypasses cache (stale during midnight reset)
...
reset_device_state() {
	local device_id="$1"
	# Cache (ALL_STATES) doesn't reflect RPC updates processed earlier in this cycle
